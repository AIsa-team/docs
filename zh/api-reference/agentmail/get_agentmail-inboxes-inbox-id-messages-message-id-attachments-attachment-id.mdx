...
获取单封邮件上某个附件的元数据。返回 `attachment_id`、`filename`、`size`、`content_type`、`content_disposition`、`content_id`，以及一个短时效的 `download_url` 和 `expires_at`。字节内容不内联，需在过期前去取 `download_url`。附件 id 来自 `get_agentmail_inbox_message` 返回的 `attachments` 数组。会话级和草稿级的孪生工具是 `get_agentmail_inbox_thread_attachment` 与 `get_agentmail_inbox_draft_attachment`。
...
//   https://aisa.one/docs/zh/api-reference/agentmail/get_agentmail-inboxes-inbox-id-messages-message-id-attachments-attachment-id.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/agentmail/get_agentmail-inboxes-inbox-id-messages-message-id-attachments-attachment-id)
