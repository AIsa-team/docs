...
description: "通过标准 POST /v1/images/generations 端点使用 Seedream、Wan 和 gpt-image-2 生成图像。采用 OpenAI 兼容的请求结构；Seedream 路由要求图像最小尺寸为 3,686,400 像素（例如 1920×1920）。"
openapi: "openapi/zh/openai-images-generations.json POST /images/generations"
---

标准的兼容 OpenAI 的 `POST /v1/images/generations` 端点。截至 2026 年 7 月 28 日，该端点服务 `seedream-5-0-260128`、`wan2.7-image`、`wan2.7-image-pro` 和 `gpt-image-2`。较早的 `seedream-4-5-251128` 需通过[通过 Chat 生成图像](/zh/api-reference/chat/post_chat-completions-image-generation)调用。

## 路由概览

| 模型 | 端点 |
|---|---|
| `seedream-5-0-260128` | **`POST /v1/images/generations`** *（本页）*，也支持 `POST /v1/chat/completions` |
...
| 模型 | 费用 | 说明 |
|---|---|---|
| `seedream-5-0-260128` | $0.035 / 次请求 | 最小图像尺寸为 **3,686,400 像素**（例如 1920×1920） |
| `wan2.7-image` | $0.030 / 次请求 | 同时支持 `POST /v1/images/edits` |
| `wan2.7-image-pro` | $0.075 / 次请求 | 更高质量的 Wan 路由 |
| `gpt-image-2` | $0.03 / 次请求，或 $0.05 / 张（1024×1024） | 部分路由同时按 token 计费，详见 [AI 模型价格](/zh/guides/pricing/ai-model-pricing-llm-inference) |
...
| `n` | integer | 否 | 图像数量。每张图像按该模型的单次请求价格单独计费 |
| `size` | string | 否 | `WIDTHxHEIGHT`。Seedream 路由必须满足 `width × height ≥ 3,686,400`；`wan2.7-image` 和 `gpt-image-2` 接受更小的尺寸（`1024x1024` 已实测可用） |
