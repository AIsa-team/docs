...
description: "排队查询某个具体 Google Shopping 商品，返回任务 id。"
...
排队查询某个具体 Google Shopping 商品，返回任务 `id`。用 `product_id` 加上从 `get_dataforseo_merchant_google_products_fetch` 结果里拿到的 `gid` 或 `data_docid` 定位商品。本家族全程异步：submit 在 `tasks[0].id` 返回任务 `id`，结果就绪后由 fetch 取回。💰 **费用发生在 submit**（实测 $0.001–0.0015），fetch 免费，重复 fetch 也不再收费。 用 `get_dataforseo_merchant_google_product_info_fetch` 取结果。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/post_dataforseo-merchant-google-product-info-task-post.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/post_dataforseo-merchant-google-product-info-task-post)
