...
description: "可用的 ChatGPT 模型，每项含 model_name、reasoning、web_search_supported 和 task_post_supported。"
...
可用的 ChatGPT 模型，每项含 `model_name`、`reasoning`、`web_search_supported` 和 `task_post_supported`。实测 5.2 KB（四份模型列表里最大的一份）。免费：上游成本为 0。**调用前先读它**——不在列表里的 `model_name` 会以 `status_code` 40501 被拒，而 HTTP 仍是 200；`task_post_supported` 则告诉你排队版本能不能用。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/get_dataforseo-ai-optimization-chat-gpt-llm-responses-models.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/get_dataforseo-ai-optimization-chat-gpt-llm-responses-models)
