...
description: "助手回答关于你的目标时最常引用的域名：total 和 items。"
...
助手回答关于你的目标时最常引用的域名：`total` 和 `items`。实测 20.3 KB，mentions 家族最大的响应。🔴 **实测上游 $0.101，约为我们收费的八倍**——本 provider 里最贵的一档。⚠️ `target` 是**对象数组**，每项形如 `{"domain": "..."}` 或 `{"keyword": "..."}`；传裸字符串会被判类型错，传字符串数组会报「Each target item must be an object」。过滤字段见 `get_dataforseo_ai_llm_mentions_available_filters`。 响应包在信封里：数据在 `tasks[0].result`，成败在 `tasks[0].status_code`——请求被拒时 HTTP 仍是 200。 **这就是该去谈的名单**：这些页面在塑造助手怎么说你。要具体页面而不是域名，用 `post_dataforseo_ai_llm_mentions_top_pages_live`。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/post_dataforseo-ai-optimization-llm-mentions-top-domains-live.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/post_dataforseo-ai-optimization-llm-mentions-top-domains-live)
