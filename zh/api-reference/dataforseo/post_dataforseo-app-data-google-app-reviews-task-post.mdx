...
description: "按 app_id 排队获取单个 Google Play 应用的评价，返回任务 id。"
...
按 `app_id` 排队获取单个 Google Play 应用的评价，返回任务 `id`。`depth` 决定抓多少条。异步：submit 在 `tasks[0].id` 返回任务 `id`，结果就绪后由 fetch 取回。💰 **费用发生在 submit**（实测 $0.0012），fetch 免费，重复 fetch 也不再收费。 用 `get_dataforseo_app_google_app_reviews_fetch` 取结果。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/post_dataforseo-app-data-google-app-reviews-task-post.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/post_dataforseo-app-data-google-app-reviews-task-post)
