...
description: "与 post_dataforseo_ai_gemini_llm_scraper_live 相同的 Gemini 答案，返回原始 HTML。"
...
与 `post_dataforseo_ai_gemini_llm_scraper_live` 相同的 Gemini 答案，返回原始 HTML。`expand_citations` 展开来源列表。响应包在信封里：数据在 `tasks[0].result`，成败在 `tasks[0].status_code`——请求被拒时 HTTP 仍是 200。 ⚠️ 比 26.1 KB 的解析版大，只有需要解析器丢掉的标记时才取。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/post_dataforseo-ai-optimization-gemini-llm-scraper-live-html.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/post_dataforseo-ai-optimization-gemini-llm-scraper-live-html)
