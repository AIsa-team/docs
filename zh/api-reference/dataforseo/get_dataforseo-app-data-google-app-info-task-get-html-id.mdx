...
description: "取回单个 Google Play 商店页的原始 HTML。"
...
取回单个 Google Play 商店页的**原始 HTML**。🔴 量级与搜索 HTML 孪生实测的 1.3 MB 相当，而解析版是几十 KB。免费——费用在 submit。响应包在信封里：数据在 `tasks[0].result`，成败在 `tasks[0].status_code`——请求被拒时 HTTP 仍是 200。 解析版是 `get_dataforseo_app_google_app_info_fetch`。
...
//   https://aisa.one/docs/zh/api-reference/dataforseo/get_dataforseo-app-data-google-app-info-task-get-html-id.md
...
[在你的 agent 里把这个端点跑起来 →](https://aisa.one/mcp?from=/zh/api-reference/dataforseo/get_dataforseo-app-data-google-app-info-task-get-html-id)
