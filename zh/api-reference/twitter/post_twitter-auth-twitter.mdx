...
<Steps>
  <Step title="启动流程">
    使用您的 AIsa API 密钥发送 `POST /apis/v1/twitter/auth_twitter`。密钥需要**同时**放在 `Authorization: Bearer` 请求头**和**请求正文的必填字段 `aisa_api_key` 中——OAuth 会话会绑定到该密钥。可选地在正文中包含 `scopes` 数组，以请求比默认更窄的权限集合。
    ...
    <Warning>
      请使用精确路径 `/apis/v1/twitter/auth_twitter`——小写、**下划线**（不是 `auth-twitter`）、版本 **v1**、方法 **POST**。连字符路径、`v2` 路径或 `GET` 请求都会返回 `404`/`405`；缺少正文字段 `aisa_api_key` 会返回 `422`。
    </Warning>
  </Step>
  <Step title="打开授权 URL">
