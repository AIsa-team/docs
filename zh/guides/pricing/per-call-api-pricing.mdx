...
<Warning>
  **重要（IMPORTANT）：** 本页不代表所有数据 API 都是固定价格。SimilarWeb 端点可能是固定计费、按返回行数计费，或按请求维度计费。发起付费 SimilarWeb 请求前，请先阅读 [SimilarWeb 价格与成本控制](/zh/guides/pricing/similarweb)。
</Warning>
