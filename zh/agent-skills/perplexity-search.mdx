...
<Tip>
  深度研究可以接受更长的处理时间，Python 客户端也实现了自动重试。对时效敏感的请求，建议降级使用 `sonar-pro` 或 `sonar-reasoning-pro`，不要长时间等待。
</Tip>
...
<CardGroup cols={3}>
  <Card title="Perplexity API 参考" icon="sparkles" href="/api-reference/perplexity/post_perplexity-sonar">
