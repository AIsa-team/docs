...
<IMPORTANT id="paid-api-approval-first">
  <before-paid-data-call>
    ...
    对准备执行的原请求原样发送，并加上 X-AISA-Cost-Mode: quote；它只估价，不执行数据调用。不要自己用 x-aisa-pricing 计算 USD。
    用户要求调研或取数不等于批准：说明精确 scope 和 quote 得到的 USD，然后等待批准。
    例外：若当前消息已明确批准一个具名数据项并给出 USD 上限，且 quote 不超过该上限，可以只执行这一项；其他情况都不得执行。
    scope 变化或 quote 超出预算时，重新 quote 并等待。若 provider 控制的价格没有已记录的最大值，不得执行；改为给出有界方案。
  </before-paid-data-call>
  <prohibited>
...
**典型场景 — SimilarWeb：** 对准备执行的请求加 `X-AISA-Cost-Mode: quote`，不要根据 [`x-aisa-pricing`](https://aisa.one/openapi.yaml) 心算。若用户已批准 0.10 美元的单月 visits 预览：Traffic & Engagement（`metrics=visits`、一个月）的 quote 不超过 0.10 美元时，只执行这一笔。若 snapshot 的 quote 没有已记录的最大值，改给有界方案。
