...
**典型场景 — SimilarWeb：** 对准备执行的请求加 `X-AISA-Cost-Mode: quote` 拿到 USD，再据此请求批准。不要自己根据 [`x-aisa-pricing`](https://aisa.one/openapi.yaml) 心算。若用户只要 0.10 美元的单月 visits 预览，应 quote Traffic & Engagement（`metrics=visits`、一个月），不要用 snapshot。若 snapshot 的 quote 没有 `max_cost_micros_usd`，不得执行。
