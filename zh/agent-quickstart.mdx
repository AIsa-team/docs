...
<IMPORTANT id="paid-api-approval-first">
  <before-paid-data-call>
    ...
    若 may_exceed_estimate 为 true 且没有 max_cost_micros_usd，只说明结算可能浮动，本身不是禁令；要看计费维度：当所有计费维度都是你在请求里自己设定的（指标数、月数、limit）时，该 quote 就是这一笔请求的预期费用，用户明确给出的不低于它的上限即可授权执行这一笔。当某个计费维度由 provider 控制且无已记录上限（例如 SimilarWeb snapshot / trend 的指标集）时，才属于没有已记录上限：不得执行该请求，必须说明没有已记录的最大费用。有界替代方案是新的 scope，需要新的批准。示例或用户给出的预算都不是可执行上限。
