...
### Formula-priced data APIs

Some paid data APIs, including SimilarWeb, use a credit formula rather than a fixed per-call price. Read the matching endpoint documentation and its machine-readable pricing declaration before a paid request; calculate the selected scope only when it has a documented maximum, then obtain explicit approval.
