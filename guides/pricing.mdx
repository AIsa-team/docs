...
### SimilarWeb: formula-based pricing

SimilarWeb is a paid data API with a credit formula. Depending on the endpoint, cost can be fixed, driven by request dimensions, or driven by returned rows. Read [SimilarWeb Pricing & Cost Control](/guides/pricing/similarweb) before a paid request; it shows the current formula, cost driver, example, and endpoint-specific control.
