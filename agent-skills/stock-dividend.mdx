...
**Production skill details from the AIsa agent-skills registry.** Analyze dividend metrics for one or more tickers using the AIsa API. This is a read-only research helper: it does not connect to brokerage accounts, place orders, make purchases, or manage portfolios.
