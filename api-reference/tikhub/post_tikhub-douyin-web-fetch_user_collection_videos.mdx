---
title: "Get user collection video data"
description: "获取用户收藏作品数据/Get user collection video data"
openapi: "openapi/tikhub.json POST /tikhub/douyin/web/fetch_user_collection_videos"
---

获取用户收藏作品数据/Get user collection video data

**Pricing** — base $0.00145 per successful call (provider cost × 1.45; final charge scales by your plan multiplier). Charged only when the upstream response body `code` is `200`.

<Note>
Response structure is not yet fully documented — **以实际调用为准 / refer to the actual API response**.
</Note>

## Example

```bash
curl -X POST "https://api.aisa.one/apis/v1/tikhub/douyin/web/fetch_user_collection_videos" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'  # see request schema below
```
