---
title: "Get comprehensive search results of specified keywords"
description: "Get comprehensive search results of specified keywords"
openapi: "openapi/tikhub-en.json GET /tikhub/tiktok/app/v3/fetch_general_search_result"
---

Get comprehensive search results of specified keywords

**Pricing** — base $0.00145 per successful call (provider cost × 1.45; final charge scales by your plan multiplier). Charged only when the upstream response body `code` is `200`.

## Example

```bash
curl "https://api.aisa.one/apis/v1/tikhub/tiktok/app/v3/fetch_general_search_result?keyword=cosplay" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
