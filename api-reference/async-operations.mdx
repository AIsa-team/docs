...
description: "How AIsa handles long-running API calls — create a task, poll its status, and download the result. Used by video generation and other heavyweight endpoints."
---
...
```bash
curl "https://api.aisa.one/apis/v1/services/aigc/tasks?task_id=a1b2c3d4-e5f6-7890-abcd-ef1234567890" \
  -H "Authorization: Bearer $AISA_API_KEY"
```
...
```json
{
  "output": {
    ...
    "video_url": "https://cdn.aisa.one/tasks/a1b2.../out.mp4",
    "video_url_expires_at": "2026-04-17T23:00:00Z"
  }
}
```
...
```bash
curl -L "https://cdn.aisa.one/tasks/a1b2.../out.mp4" -o output.mp4
```
...
| `RUNNING`    | Actively generating. May include a `progress` (0.0–1.0) field    | Billed on success            |
| `SUCCEEDED`  | Generation complete. Asset URL available for download            | Billed in full               |
| `FAILED`     | Generation failed. `error.message` explains why                  | **Not billed**               |
| `CANCELLED`  | Task was cancelled (e.g., user request or policy)                | Partial billing possible     |
| `UNKNOWN`    | Transient state — poll again in a few seconds                    | —                            |
...
def wait_for_task(task_id, interval=3, timeout=300):
    ...
    while time.time() < deadline:
        r = requests.get(f"{API}/services/aigc/tasks", params={"task_id": task_id}, headers=HEADERS)
        r.raise_for_status()
        out = r.json()["output"]
        ...
        if status == "FAILED":
            raise RuntimeError(f"Task failed: {out.get('error', {})}")
        time.sleep(interval)
