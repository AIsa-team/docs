...
The standard OpenAI-compatible `POST /v1/images/generations` endpoint. Currently routed to **ByteDance Seedream** (`seedream-4-5-251128`). Wan 2.7 models do **not** use this path — see [Image Generation via Chat](/api-reference/chat/post_chat-completions-image-generation).
...
| `wan2.7-image` / `wan2.7-image-pro` | [`POST /v1/chat/completions`](/api-reference/chat/post_chat-completions-image-generation) |
