...
**Classic example — SimilarWeb:** quote the intended request with `X-AISA-Cost-Mode: quote`, then ask for approval using that USD. Do not self-calculate from [`x-aisa-pricing`](https://aisa.one/openapi.yaml). For a $0.10 one-month visits preview, quote Traffic & Engagement (`metrics=visits`, one month); do not use snapshot. If snapshot quote has no `max_cost_micros_usd`, do not execute it.
